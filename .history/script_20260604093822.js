/**
 * Levelup.wr | LUXURY STREETWEAR 2026
 * MAIN SCRIPT
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const STORAGE_KEY = 'levelupwr-theme';
...
if (Firewall.isAdmin()) {
            ...
            <div class="product-card" data-id="${p.id}" data-name="${p.name}" data-price="${p.price}" data-img="${p.img}">
              ...
              <div class="product-info">
                <div class="product-info-left">
                  ...
                  <div class="product-desc">Premium quality streetwear essential from the Levelup.wr collection.</div>
                </div>
  ...
  // 4. CART SYSTEM
  let cart = JSON.parse(localStorage.getItem("levelupwr_cart") || "[]");
  ...
  const saveCart = () => {
    localStorage.setItem("levelupwr_cart", JSON.stringify(cart));
  };
  ...
  // ============ WISHLIST SYSTEM ============
  let wishlist = JSON.parse(localStorage.getItem("levelupwr_wishlist") || "[]");

  const saveWishlist = () => {
    localStorage.setItem("levelupwr_wishlist", JSON.stringify(wishlist));
