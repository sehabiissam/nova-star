...
if (Firewall.isAdmin()) {
    ...
    const productToHTML = (p) => {
            ...
            <div class="product-card" data-id="${p.id}" data-name="${p.name}" data-price="${p.price}" data-img="${p.img}">
              ...
              <div class="product-info">
                <div class="product-info-left">
                  <div class="product-brand">${getCategoryLabel(p.categoryId, p.category) || "UMNUN"}</div>
                  <div class="product-name">${p.name}</div>
                  <div class="product-category-pill">${getCategoryLabel(p.categoryId, p.category) || "Essential"}</div>
                  <div class="product-desc">Premium quality streetwear essential from the UMNUN collection.</div>
                </div>
      ...
      return `
            <div class="product-card" data-id="${p.id}" data-name="${p.name}" data-price="${p.price}" data-img="${p.img}">
              ...
              <div class="product-info">
                <div class="product-brand">${getCategoryLabel(p.categoryId, p.category) || "Levelup.wr"}</div>
                <div class="product-name">${p.name}</div>
  ...
  // 4. CART SYSTEM
  let cart = JSON.parse(localStorage.getItem("umnun_cart") || "[]");
  ...
  const saveCart = () => {
    localStorage.setItem("umnun_cart", JSON.stringify(cart));
  };
  ...
  // ============ WISHLIST SYSTEM ============
  let wishlist = JSON.parse(localStorage.getItem("umnun_wishlist") || "[]");

  const saveWishlist = () => {
    localStorage.setItem("umnun_wishlist", JSON.stringify(wishlist));
    ...
    grid.innerHTML = wishlist
      .map(
        ...
        <div class="product-card" data-id="${item.id}" data-name="${item.name}" data-price="${item.price || 0}" data-img="${item.img}">
          ...
          <div class="product-info">
            <div class="product-brand">${item.category || "Levelup.wr"}</div>
    ...
    container.innerHTML = `
      ...
      <div class="product-view-layout product-view">
        ...
        <div class="product-view-details">
          <div>
            <span class="product-view-brand">Levelup.wr</span>
          ...
          <div class="product-view-description">
            ...
            <div class="product-view-description-text">${product.description || "Premium quality streetwear essential from the Levelup.wr collection. Built for those who refuse to stay in the shadows. Each piece is crafted with precision and designed for the future."}</div>
          </div>
