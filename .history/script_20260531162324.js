...
if (Firewall.isAdmin()) {
  ...
  const subscribeEmail = async (email) => {
    ...
    try {
      ...
      // Show the real Firebase error message for debugging
      let userMessage = "Subscription failed. Please try again.";
      if (error.code === "permission-denied") {
        userMessage = "Subscription failed: Permission denied. Firestore rules may need to be updated.";
      } else if (error.code === "unavailable") {
        userMessage = "Service temporarily unavailable. Please try again.";
      } else if (error.code === "not-found") {
        userMessage = "Firestore database not found. Check configuration.";
      } else if (error.code === "invalid-argument") {
        userMessage = "Invalid data provided.";
      }
      showNewsletterFeedback(userMessage, true);
      return false;
    }
  };
