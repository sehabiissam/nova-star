...
if (Firewall.isAdmin()) {
  ...
  const showNewsletterFeedback = (message, isError = false) => {
    if (!newsletterFeedback) return;
    newsletterFeedback.textContent = message;
    newsletterFeedback.style.color = isError ? "#ff4d4d" : "var(--accent)";
  ...
  const subscribeEmail = async (email) => {
    ...
    if (!isValidEmail(normalizedEmail)) {
      ...
      showNewsletterFeedback("Please enter a valid email address.", true);
      return false;
    }

    // Check duplicate from in-memory cache (only populated if admin has loaded subscribers)
    const existing = subscribers.find(
      (s) => s.email && normalizeEmail(s.email) === normalizedEmail
    );
    if (existing) {
      console.log("[NEWSLETTER] Duplicate subscription attempt:", normalizedEmail);
      showNewsletterFeedback("You are already subscribed.", true);
      return false;
    }
    ...
    try {
      ...
      // Use toISOString() consistent with every other collection in this project
      // (orders, products, reviews, logs, trash, categories all use string ISO dates)
      // serverTimestamp() sentinels can cause issues with named databases
      const docRef = await addDoc(subscribersRef, {
        email: normalizedEmail,
        subscribedAt: new Date().toISOString(),
      });
      console.log("[NEWSLETTER] Subscribe SUCCESS - doc ID:", docRef.id);
      showNewsletterFeedback("Thanks for joining ZetSpace. You'll be notified when new products arrive.");
      if (newsletterEmail) newsletterEmail.value = "";
      return true;
    } catch (error) {
      console.error("[NEWSLETTER_SUBSCRIBE_ERROR] === DETAILED ERROR ===");
      ...
      console.error("[NEWSLETTER] Full error:", JSON.stringify(error, Object.getOwnPropertyNames(error)));
      
      // Show the REAL Firebase error message directly instead of hiding it
      const realMessage = error.code 
        ? `[${error.code}] ${error.message}` 
        : error.message || "Unknown error";
      console.error("[NEWSLETTER] Real error to display:", realMessage);
      showNewsletterFeedback(`SUBSCRIPTION FAILED: ${realMessage}`, true);
      return false;
    }
  };
  ...
  if (newsletterForm) {
    newsletterForm.addEventListener("submit", async (e) => {
      ...
      if (!email) {
        showNewsletterFeedback("Please enter your email address.", true);
        return;
      }
      ...
      if (newsletterSubmitBtn) {
        ...
        newsletterSubmitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> SUBSCRIBING...';
      }
      ...
      if (newsletterSubmitBtn) {
        ...
        newsletterSubmitBtn.innerHTML = 'JOIN';
      }
    });
  }

  // Subscribers listener (Admin Only)
  const startSubscribersListener = () => {
    if (activeListeners.subscribers) activeListeners.subscribers();

    if (!Firewall.isAdmin()) {
      activeListeners.subscribers = null;
      return;
    }

    activeListeners.subscribers = onSnapshot(
      collection(db, "newsletter_subscribers"),
      (snapshot) => {
        subscribers = snapshot.docs.map((d) => ({
          ...d.data(),
          id: d.id,
        }));
        // Sort by subscribedAt descending in JavaScript
        subscribers.sort((a, b) => {
          const dateA = a.subscribedAt ? new Date(a.subscribedAt) : new Date(0);
          const dateB = b.subscribedAt ? new Date(b.subscribedAt) : new Date(0);
          return dateB - dateA;
        });
        renderAdminSubscribers();
      },
      (err) => {
        console.warn("[SYSTEM] SUBSCRIBERS_LISTENER_ERR:", err.message);
        handleFirestoreError(err, OperationType.LIST, "newsletter_subscribers");
      }
    );
  };

  // Render admin subscribers tab
  const renderAdminSubscribers = () => {
    ...
    if (badge) {
      badge.textContent = `${subscribers.length} SUBSCRIBER${subscribers.length !== 1 ? 'S' : ''}`;
    }

    if (subscribers.length === 0) {
      list.innerHTML =
        '<tr><td colspan="3" style="text-align:center; opacity: 0.5; padding: 2rem;">NO SUBSCRIBERS YET.</td></tr>';
      return;
    }
    ...
    list.innerHTML = [...subscribers]
      .map((s) => {
        ...
        let date = "N/A";
        if (s.subscribedAt) {
          if (typeof s.subscribedAt === 'object' && s.subscribedAt.seconds) {
            date = new Date(s.subscribedAt.seconds * 1000).toLocaleDateString();
          } else {
        ...
        return `
          <tr>
            <td><strong style="color:#fff;">${escapeInputValue(s.email)}</strong></td>
            <td>${date}</td>
            <td>
              <button class="action-btn delete-btn" onclick="event.stopPropagation(); window.__deleteSubscriber('${s.id}')"><i class="fa-solid fa-user-minus"></i></button>
            </td>
          </tr>
        `;
      })
  ...
  // Delete subscriber
  window.__deleteSubscriber = async (id) => {
    if (!Firewall.isAdmin()) return;
    try {
  ...
  // Integrate subscribers listener into startListeners
  const originalStartListeners = startListeners;
  // We'll hook the subscribers listener into the existing startListeners
  // Add subscribers to activeListeners object
  activeListeners.subscribers = null;

  // Override startListeners to include subscribers
  const enhancedStartListeners = () => {
    // Call the real startListeners from the closure
    // We can't easily override the existing function, so we hook into the renderAdmin
    // Instead, we call startSubscribersListener from auth state change
    startSubscribersListener();
  };

  // Hook into auth state changes: when admin logs in, start subscriber listener
  // This is already handled: onAuthStateChanged calls startListeners which triggers admin stuff
  // We need to also call startSubscribersListener when auth changes.
  // Since startListeners is called in onAuthStateChanged, we inject our subscriber listener there
  // by wrapping the existing auth listener setup after the fact.
  
  // We'll add a mutation observer or simply call startSubscribersListener from the admin view
  // The simplest approach: call it whenever renderAdmin is called
  const origRenderAdmin = renderAdmin;
  renderAdmin = function() {
    origRenderAdmin.apply(this, arguments);
    startSubscribersListener();
  };

  // Also init on DOMContentLoaded if admin already authenticated
  setTimeout(() => {
    if (Firewall.isAdmin()) {
      startSubscribersListener();
    }
