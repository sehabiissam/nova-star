...
if (Firewall.isAdmin()) {
  ...
  // 4. CART SYSTEM
  let cart = JSON.parse(localStorage.getItem("91s-store_cart") || "[]");
  ...
  const saveCart = () => {
    localStorage.setItem("91s-store_cart", JSON.stringify(cart));
  };
  ...
  // ============ WISHLIST SYSTEM ============
  let wishlist = JSON.parse(localStorage.getItem("91s-store_wishlist") || "[]");

  const saveWishlist = () => {
    localStorage.setItem("91s-store_wishlist", JSON.stringify(wishlist));
    ...
    grid.innerHTML = wishlist
      .map(
        ...
        <div class="product-card" data-id="${item.id}" data-name="${item.name}" data-price="${item.price || 0}" data-img="${item.img}">
          ...
          <div class="product-info">
            <div class="product-brand">${item.category || "91s STORE"}</div>
