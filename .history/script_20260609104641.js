/**
 * 91s STORE | LUXURY STREETWEAR 2026
 * MAIN SCRIPT
 */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const STORAGE_KEY = '91s-store-theme';
...
if (Firewall.isAdmin()) {
    ...
    const productToHTML = (p) => {
            ...
            <div class="product-card" data-id="${p.id}" data-name="${p.name}" data-price="${p.price}" data-img="${p.img}">
              ...
              <div class="product-info">
                <div class="product-info-left">
                  <div class="product-brand">${getCategoryLabel(p.categoryId, p.category) || "91s STORE"}</div>
                  <div class="product-name">${p.name}</div>
                  <div class="product-category-pill">${getCategoryLabel(p.categoryId, p.category) || "Essential"}</div>
                  <div class="product-desc">Premium quality streetwear essential from the 91s STORE collection.</div>
                </div>
      ...
      return `
            <div class="product-card" data-id="${p.id}" data-name="${p.name}" data-price="${p.price}" data-img="${p.img}">
              ...
              <div class="product-info">
                <div class="product-brand">${getCategoryLabel(p.categoryId, p.category) || "JWS CLOTHING DZ"}</div>
                <div class="product-name">${p.name}</div>
  ...
  // 4. CART SYSTEM
  let cart = JSON.parse(localStorage.getItem("jwsclothingdz_cart") || "[]");
  ...
  const saveCart = () => {
    localStorage.setItem("jwsclothingdz_cart", JSON.stringify(cart));
  };
  ...
  // ============ WISHLIST SYSTEM ============
  let wishlist = JSON.parse(localStorage.getItem("jwsclothingdz_wishlist") || "[]");

  const saveWishlist = () => {
    localStorage.setItem("jwsclothingdz_wishlist", JSON.stringify(wishlist));
    ...
    grid.innerHTML = wishlist
      .map(
        ...
        <div class="product-card" data-id="${item.id}" data-name="${item.name}" data-price="${item.price || 0}" data-img="${item.img}">
          ...
          <div class="product-info">
            <div class="product-brand">${item.category || "JWS CLOTHING DZ"}</div>
    ...
    container.innerHTML = `
      ...
      <div class="product-view-layout product-view">
        ...
        <div class="product-view-details">
          <div>
            <span class="product-view-brand">91s STORE</span>
          ...
          <div class="product-view-description">
            ...
            <div class="product-view-description-text">${product.description || "Premium quality streetwear essential from the 91s STORE collection. Built for those who refuse to stay in the shadows. Each piece is crafted with precision and designed for the future."}</div>
          </div>
